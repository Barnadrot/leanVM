...
pub(super) fn exec_multi_row(
    ...
    for i in 0..size {
        ...
        let addr_res = ptr_res.to_usize();
        trace.columns[COL_IDX_A_HI].push(F::from_usize(addr_a >> half_bits));
        trace.columns[COL_IDX_A_LO].push(F::from_usize(addr_a & half_mask));
        trace.columns[COL_IDX_B_HI].push(F::from_usize(addr_b >> half_bits));
        trace.columns[COL_IDX_B_LO].push(F::from_usize(addr_b & half_mask));
        trace.columns[COL_IDX_RES_HI].push(F::from_usize(addr_res >> half_bits));
        trace.columns[COL_IDX_RES_LO].push(F::from_usize(addr_res & half_mask));
