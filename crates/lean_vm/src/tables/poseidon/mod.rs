...
// 27 committed: 9 flags/control + 2 d=2 decomposition + 16 inputs.
...
impl<const BUS: bool> TableT for Poseidon16Precompile<BUS> {
    ...
    fn execute<M: MemoryAccess>(
        ...
        trace.columns[POSEIDON_COL_NU_C_LO].push(F::from_usize(nu_c_val & half_mask));
...
impl<const BUS: bool> Air for Poseidon16Precompile<BUS> {
    ...
    fn n_constraints(&self) -> usize {
        // Bus: 2, bool checks: 4, flag mutual exclusion: 1, addr checks: 2, d=2 decomp: 1, compression: 2*8=16
        2 * BUS as usize + 4 + 1 + 2 + 1 + 2 * (WIDTH / 2)
    }
    fn eval<AB: AirBuilder>(&self, builder: &mut AB, extra_data: &Self::ExtraData) {
        ...
        let half_bits_mod = AB::F::from_usize(1usize << (MAX_LOG_MEMORY_SIZE / 2));
        let decomp_nu_c = cols.nu_c - cols.nu_c_hi * half_bits_mod - cols.nu_c_lo;
